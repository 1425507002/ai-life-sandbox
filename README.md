...
阶段 3 已完成：剧本世界、内部地图、多条人生存档、回档和四套 UI 主题均已接入。阶段 4 的长期记忆压缩、事件序号回放、上下文压力测试和 AI 增强超时降级也已完成当前原型范围。当前仍是本地优先原型：第一阶段暂不做账号、多人、云同步、复杂战斗和既有 IP 内容；跨设备使用请通过 JSON 导出/导入。
