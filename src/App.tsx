...
import { verifiedScriptCatalog } from './data/scriptCatalog'
...
function App() {
  const { scripts, sessions, activeScriptId, activeLifeId, activeNav, providerConfig, actionMode, uiThemeId, hydrated, lastAction, lastNotice, scriptDraft, selectScript, selectLife, setNav, setActionMode, setUiTheme, runAction, setProviderConfig, hydrate, startNewLife, updatePlayer, rollbackLife, notify, clearNotice, importRuntime, importScriptPackage, generateScriptStage, clearScriptDraft, getExportPayload } = useGameStore()
  ...
  return (
    <div className="app-shell" style={rootStyle}>
      ...
      <main className="main-area">
        ...
        {activeNav === 'settings' && <SettingsScreen scripts={scripts} activeScriptId={activeScriptId} onSelectScript={selectScript} script={script} actionMode={actionMode} onActionModeChange={setActionMode} uiThemeId={uiThemeId} onUiThemeChange={setUiTheme} providerConfig={providerConfig} onProviderChange={setProviderConfig} scriptDraft={scriptDraft} onGenerateScriptStage={generateScriptStage} onClearScriptDraft={clearScriptDraft} onLoadScriptDraft={(draft) => { if (draft.script) importScriptPackage(draft.script) }} onExport={() => downloadSave(getExportPayload())} onImport={() => fileInput.current?.click()} onImportScript={() => scriptInput.current?.click()} onReset={() => setNewLifeOpen(true)} />}
...
function StatusCard({ state }: { state: GameState }) {
  return <section className="rail-card status-card"><div className="rail-card-header"><span className="section-kicker"><UserRound size={15} /> 你</span><span className="status-live">在线</span></div><div className="avatar-row"><div className="avatar avatar-large">{state.player.name.slice(0, 1)}</div><div><strong>{state.player.name}</strong><span>{state.player.role} · {state.player.age} 岁</span></div><ArrowUpRight size={16} /></div><div className="role-line"><BriefcaseBusiness size={14} /><span>{state.player.profession}</span><span className="role-separator" /> <HeartHandshake size={14} /><span>{state.player.mood}</span></div><StatBar icon={<Zap size={14} />} label="体力" value={state.player.stamina} max={state.player.maxStamina} tone="green" /><StatBar icon={<ShieldCheck size={14} />} label="状态" value={state.player.health} max={state.player.maxHealth} tone="coral" /><div className="money-line"><WalletCards size={15} /><span>随身钱币</span><strong>{money(state.player.money)}</strong></div></section>
}

function StatBar({ icon, label, value, max, tone }: { icon: React.ReactNode; label: string; value: number; max: number; tone: string }) { const ratio = max > 0 ? Math.max(0, Math.min(100, (value / max) * 100)) : 0; return <div className="stat-line"><div className="stat-label">{icon}<span>{label}</span></div><div className="stat-track"><span className={tone} style={{ width: `${ratio}%` }} /></div><strong>{value}/{max}</strong></div> }
...
function PeopleScreen({ state }: { state: GameState }) { const knownNpcs = state.npcs.filter((npc) => npc.met === true); return <div className="page"><PageHeader eyebrow="人物关系" title="他们也在过自己的生活" copy="只有真正相遇并留下经历的人，才会出现在这里。世界里的陌生人不会自动成为你的关系。" />{knownNpcs.length ? <div className="people-grid">{knownNpcs.map((npc) => <article className="person-card" key={npc.id}><div className="person-head"><div className="avatar">{npc.avatar}</div><div><h3>{npc.name}</h3><span>{npc.role}</span></div><span className="person-score">+{npc.relationship}</span></div><p>{npc.summary}</p><div className="person-status"><span><Clock3 size={13} /> {npc.status}</span><small>最近：{npc.lastInteraction}</small></div><div className="relationship-track"><span style={{ width: `${Math.min(100, npc.relationship)}%` }} /></div></article>)}</div> : <div className="empty-state"><UsersRound size={28} /><strong>还没有认识这里的人</strong><p>先从当前年龄可以完成的行动开始。等一次真实相遇发生后，人物卡会自动出现在这里。</p></div>}</div> }

function HistoryScreen({ state, onRollback }: { state: GameState; onRollback: (turn: number) => void }) { return <div className="page"><PageHeader eyebrow="人生履历" title="发生过的事，会留下来" copy="行动日志按时间保存。失败、错过和没有定论的事情同样属于你的人生。回到较早记录会保留当前人生，但撤销之后的变化。" /><section className="timeline">{state.history.map((event) => <article className="timeline-item" key={event.id}><div className="timeline-marker" /><div className="timeline-date">{event.date}{event.sequence !== undefined && <span> · 事件 #{event.sequence}</span>}</div><div className="timeline-content"><div className="timeline-title"><h3>{event.title}</h3><span className={`outcome-tag ${event.outcome}`}>{event.outcome === 'success' ? '已完成' : event.outcome === 'partial' ? '部分完成' : event.outcome === 'unknown' ? '待确认' : '未执行'}</span></div>{event.input && <p className="timeline-input"><strong>你的行动：</strong>{event.input}</p>}<p>{event.body}</p>{event.stateDiff?.length ? <div className="state-diff" aria-label="状态变化">{event.stateDiff.slice(0, 6).map((diff) => <span key={diff.key}>{diff.label} {diff.before} → {diff.after}</span>)}</div> : null}<div className="tag-row">{event.tags.map((tag, index) => <span key={`${tag}-${index}`}>{tag}</span>)}</div>{event.turn !== undefined && event.turn < state.turn && <button className="history-rollback" type="button" onClick={() => onRollback(event.turn!)}><RotateCcw size={13} /> 回到第 {event.turn} 次记录</button>}</div></article>)}</section></div> }

function MapScreen({ state }: { state: GameState }) {
  const knownLocations = state.locations.filter((location) => location.discovered !== false)
  const undiscoveredCount = Math.max(0, state.locations.length - knownLocations.length)
  const currentPlace = state.world.location.split(' · ').at(-1)
  return <div className="page"><PageHeader eyebrow="当前世界地图" title={state.world.region} copy="你出生时只知道身边的一小块地方。探索、传闻和真实相遇，会让地图一点点长出来。" /><div className="map-discovery-summary"><span><Compass size={15} /> 已知地点</span><strong>{knownLocations.length} / {state.locations.length}</strong><small>{undiscoveredCount ? `还有 ${undiscoveredCount} 个地点藏在未探索的雾里` : '这片生活圈暂时已经全部展开'}</small></div><div className="map-layout"><section className="map-canvas"><div className="map-water water-one" /><div className="map-water water-two" /><div className="map-road road-one" /><div className="map-road road-two" /><div className="map-hill map-hill-one" /><div className="map-hill map-hill-two" /><div className="map-label label-home"><Home size={14} />{state.world.location.split(' · ')[0]}</div>{knownLocations.map((location, index) => { const Icon = iconForLocation(location.kind); return <button key={location.id} className={`map-pin pin-${index % 4} ${location.name === currentPlace ? 'current' : ''}`} onClick={() => undefined}><span><Icon size={14} /></span><strong>{location.name}</strong></button> })}{undiscoveredCount > 0 && <div className="map-fog-note"><span>?</span><strong>未探索区域</strong><small>继续生活，地图会自己展开</small></div>}<div className="map-compass"><span>N</span><Compass size={38} /></div></section><aside className="location-list"><div className="panel-title"><span>已知地点</span><span className="panel-meta">{knownLocations.length} / {state.locations.length}</span></div>{knownLocations.map((location) => { const Icon = iconForLocation(location.kind); return <div className="location-row" key={location.id}><div className="location-icon"><Icon size={16} /></div><div><strong>{location.name}</strong><span>{location.distance} · {location.kind}</span></div><ChevronRight size={16} /></div> })}{undiscoveredCount > 0 && <div className="location-unknown"><span className="unknown-mark">?</span><div><strong>还有未知地点</strong><span>探索、传闻或事件可能带来新的坐标</span></div></div>}</aside></div></div>
}

function ScriptLibraryPanel({ scripts, activeScriptId, onSelectScript }: { scripts: ScriptPackage[]; activeScriptId: string; onSelectScript: (scriptId: string) => void }) {
  const visibleScripts = scripts.filter((item) => item.manifest.id === 'western-world' || verifiedScriptCatalog.some((entry) => entry.id === item.manifest.id) || item.manifest.id === activeScriptId)
  return <section className="settings-panel script-library-panel"><div className="settings-heading"><div className="settings-icon sky"><BookOpen size={18} /></div><div><h2>剧本库</h2><p>只把达到 85 分门槛的预设剧本加入可加载目录；地图仍属于剧本内部。</p></div></div><div className="script-library-grid">{visibleScripts.map((item) => { const catalog = verifiedScriptCatalog.find((entry) => entry.id === item.manifest.id); const isActive = item.manifest.id === activeScriptId; return <article className={`script-library-card ${isActive ? 'active' : ''}`} key={item.manifest.id}><div className="script-card-top"><span className="script-card-category">{catalog?.category ?? '当前基础世界'}</span><strong>{catalog ? `${catalog.score} 分通过` : '运行基线'}</strong></div><h3>{item.manifest.title}</h3><p>{item.manifest.description}</p><div className="script-card-meta">{item.manifest.capabilities.slice(0, 4).map((capability) => <span key={capability}>{capability}</span>)}</div><button className={isActive ? 'quiet-button compact' : 'primary-button compact'} type="button" disabled={isActive} onClick={() => onSelectScript(item.manifest.id)}>{isActive ? '当前正在游玩' : '加载这个剧本'}</button></article> })}</div><div className="settings-note"><ShieldCheck size={15} /><span>《江湖／修行人生》《灾后聚落人生》《航海／边境拓荒人生》目前只保留概念稿，不会提前出现在游戏中。</span></div></section>
}

const scriptGenerationStages: Array<{ id: ScriptGenerationStage; label: string }> = [
  { id: 'world', label: '世界骨架' },
  { id: 'maps', label: '地图与地点' },
  { id: 'characters', label: '人物与身份' },
  { id: 'actions', label: '行动与规则' },
  { id: 'events', label: '事件与开场内容' },
]

function ScriptGenerationPanel({ scriptDraft, onGenerate, onClear, onLoad }: { scriptDraft: ScriptGenerationReport | null; onGenerate: (stage: ScriptGenerationStage, preferences?: ScriptGenerationPreferences) => Promise<void>; onClear: () => void; onLoad: (draft: ScriptGenerationReport) => void }) {
  const [stage, setStage] = useState<ScriptGenerationStage>('world')
  const [busy, setBusy] = useState(false)
  const generate = async () => { setBusy(true); await onGenerate(stage); setBusy(false) }
  const generatedScript = scriptDraft?.script
  return <section className="settings-panel script-generation-panel"><div className="settings-heading"><div className="settings-icon warm"><Sparkles size={18} /></div><div><h2>AI 剧本生成草稿</h2><p>按阶段生成、预览和确认。草稿不会直接写入当前人生。</p></div></div><div className="script-generation-controls"><label>生成阶段<select aria-label="AI剧本生成阶段" value={stage} onChange={(event) => setStage(event.target.value as ScriptGenerationStage)}>{scriptGenerationStages.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><button className="primary-button" type="button" disabled={busy} onClick={() => void generate()}>{busy ? '正在生成…' : '生成阶段草稿'}</button></div>{scriptDraft && <div className={`script-draft-preview ${scriptDraft.valid ? 'valid' : 'invalid'}`} role="status"><div className="draft-header"><div><span className="section-kicker"><Sparkles size={14} /> 生成结果</span><strong>{scriptDraft.valid ? '已通过校验，可预览' : '未通过校验，未加载'}</strong></div><button className="icon-button" type="button" aria-label="清除剧本草稿" onClick={onClear}><X size={15} /></button></div>{scriptDraft.valid && generatedScript ? <><p className="draft-summary">已生成：{scriptDraft.changedKeys.join('、') || '局部内容'}。当前剧本 ID 保持为 <code>{generatedScript.manifest.id}</code>。</p><div className="draft-facts"><span>地图 {generatedScript.maps?.length ?? 0}</span><span>人物 {generatedScript.world.seedState.npcs.length}</span><span>行动 {Object.values(generatedScript.ageStageActions ?? {}).flat().length}</span><span>事件 {generatedScript.events?.length ?? 0}</span></div><button className="wide-button" type="button" onClick={() => onLoad(scriptDraft)}><Check size={15} /> 确认并加载为剧本版本</button></> : <div className="draft-errors">{scriptDraft.errors.slice(0, 3).map((error) => <span key={error}><CircleAlert size={14} /> {error}</span>)}</div>}{scriptDraft.warnings.length > 0 && <div className="draft-warnings">{scriptDraft.warnings.map((warning) => <span key={warning}>提示：{warning}</span>)}</div>}</div>}</section>
}

function SettingsScreen({ scripts, activeScriptId, onSelectScript, script, actionMode, onActionModeChange, uiThemeId, onUiThemeChange, providerConfig, onProviderChange, scriptDraft, onGenerateScriptStage, onClearScriptDraft, onLoadScriptDraft, onExport, onImport, onImportScript, onReset }: { scripts: ScriptPackage[]; activeScriptId: string; onSelectScript: (scriptId: string) => void; script: ScriptPackage; actionMode: ActionGenerationMode; onActionModeChange: (mode: ActionGenerationMode) => void; uiThemeId: UiThemeId; onUiThemeChange: (themeId: UiThemeId) => void; providerConfig: ProviderConfig; onProviderChange: (config: Partial<ProviderConfig>) => void; scriptDraft: ScriptGenerationReport | null; onGenerateScriptStage: (stage: ScriptGenerationStage, preferences?: ScriptGenerationPreferences) => Promise<void>; onClearScriptDraft: () => void; onLoadScriptDraft: (draft: ScriptGenerationReport) => void; onExport: () => void; onImport: () => void; onImportScript: () => void; onReset: () => void }) {
