...
function CharacterEditor({ state, script, onClose, onSave }: { state: GameState; script: ScriptPackage; onClose: () => void; onSave: (patch: Partial<GameState['player']>) => void }) {
  ...
  const roles = selectedMap?.availableRoles?.length ? selectedMap.availableRoles : config?.roles?.length ? config.roles : [state.player.role]
  const professions = selectedMap?.availableProfessions?.length ? selectedMap.availableProfessions : config?.professions?.length ? config.professions : [state.player.profession]
...
function NewLifeModal({ state, script, onClose, onStart }: { state: GameState; script: ScriptPackage; onClose: () => void; onStart: (setup: { mapId?: string; player?: Partial<GameState['player']> }) => void }) {
  const maps = script.maps ?? []
  const ageStages = script.characterCreation?.ageStages ?? []
  ...
  const [draft, setDraft] = useState({ name: state.player.name, age: String(state.player.age), ageStage: initialStage, role: state.player.role, profession: state.player.profession, traits: state.player.traits })
  const roles = selectedMap?.availableRoles?.length ? selectedMap.availableRoles : script.characterCreation?.roles ?? [state.player.role]
  const professions = selectedMap?.availableProfessions?.length ? selectedMap.availableProfessions : script.characterCreation?.professions ?? [state.player.profession]
  ...
  const setMap = (nextMapId: string) => {
    ...
    setMapId(nextMapId)
    setDraft((current) => ({ ...current, role: nextMap?.availableRoles?.includes(current.role) ? current.role : nextMap?.availableRoles?.[0] ?? current.role, profession: nextMap?.availableProfessions?.includes(current.profession) ? current.profession : nextMap?.availableProfessions?.[0] ?? current.profession }))
  }
  const setStage = (stageId: string) => {
    const stage = ageStages.find((item) => item.id === stageId)
    setDraft((current) => ({ ...current, ageStage: stageId as typeof current.ageStage, age: stage ? String(Math.max(stage.minAge, Math.min(stage.maxAge, Number(current.age) || stage.minAge))) : current.age }))
  }
  const randomize = () => {
    ...
    const nextRoles = randomMap?.availableRoles?.length ? randomMap.availableRoles : roles
    const nextProfessions = randomMap?.availableProfessions?.length ? randomMap.availableProfessions : professions
