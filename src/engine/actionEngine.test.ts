...
describe('action engine', () => {
  ...
  it('creates a clean age-aware baby life instead of reusing the demo seed', () => {
    ...
    expect(state.world.narrative.join(' ')).toContain('接受照料')
    expect(state.world.narrative.join(' ')).not.toContain('凉了一半的茶')
    expect(state.world.narrative.join(' ')).not.toContain('没有人替你安排命运')
    expect(state.world.currentFocus).toBe('先安排照料、观察和休息')
  ...
  it('starts a new life without carrying old news or headline details', () => {
    ...
    expect(state.world.headline).toBe('灰潮港的潮声隔着墙传来，你先被熟悉的脚步和微咸的空气唤醒。')
